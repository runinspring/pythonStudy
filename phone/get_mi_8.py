    ...
    for item in datas:
        values.append((getData(item, 'id'), getData(item, 'guid'),
                       ...
                       getData(item, 'userImageUrl'), getData(
                           item, 'nickname'), getData(item, 'productSize'),
                       getData(item, 'userLevelName')))

    db = pymysql.connect("localhost", "test", "123456", "JD_PHONE")
    cursor = db.cursor()
    sql = "INSERT INTO cm_mi_8 (id, guid,content,creationTime,referenceName,userImageUrl,nickname,memery,userLevelName) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) "
...
def getURL():
    ...
    url = 'https://sclub.jd.com/comment/productPageComments.action?&productId=7437764&score=0&sortType=6&page=%s&pageSize=10&isShadowSku=0&rid=0&fold=1' % idx
